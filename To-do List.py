...
def show_task_details():
    ...
    if selected_task_index:
        ...
        details_window.config(bg="#e0f7fa")

        name_label = tk.Label(details_window, text="Task Name:", bg="#e0f7fa")
        name_label.pack(pady=5)
        name_value = tk.Label(details_window, text=task["name"], bg="#e0f7fa")
        name_value.pack(pady=5)

        description_label = tk.Label(details_window, text="Description:", bg="#e0f7fa")
        description_label.pack(pady=5)
        description_entry = tk.Entry(details_window)
        description_entry.insert(0, task["description"])
        description_entry.pack(pady=5)

        due_date_label = tk.Label(details_window, text="Due Date:", bg="#e0f7fa")
...
title = tk.Label(root, text="The To-Do List", font=("Arial", 24), bg="#fce4ec", fg="#d81b60")
title.pack(pady=10)

frame = tk.Frame(root, bg="#fce4ec")
frame.pack(pady=10)

task_label = tk.Label(frame, text="Enter the Task:", font=("Arial", 12), bg="#fce4ec")
task_label.grid(row=0, column=0, padx=5, pady=5)

task_entry = tk.Entry(frame, font=("Arial", 12))
task_entry.grid(row=0, column=1, padx=5, pady=5)

add_button = tk.Button(frame, text="Add Task", command=add_task, font=("Arial", 12), bg="#f8bbd0", fg="#d81b60")
add_button.grid(row=1, column=0, padx=5, pady=5)

edit_button = tk.Button(frame, text="Edit Task", command=edit_task, font=("Arial", 12), bg="#f8bbd0", fg="#d81b60")
edit_button.grid(row=1, column=1, padx=5, pady=5)

remove_button = tk.Button(frame, text="Remove Task", command=remove_task, font=("Arial", 12), bg="#f8bbd0", fg="#d81b60")
remove_button.grid(row=2, column=0, padx=5, pady=5)

complete_button = tk.Button(frame, text="Complete Task", command=complete_task, font=("Arial", 12), bg="#f8bbd0", fg="#d81b60")
complete_button.grid(row=2, column=1, padx=5, pady=5)

details_button = tk.Button(frame, text="Show Details", command=show_task_details, font=("Arial", 12), bg="#f8bbd0", fg="#d81b60")
details_button.grid(row=3, column=0, padx=5, pady=5)

listbox = tk.Listbox(root, selectmode=tk.SINGLE, font=("Arial", 12), bg="#ffebee", fg="#d81b60")
